...
export default function Scrollable({ children, className }: ScrollableProps) {
    ...
    const onScrollContainer = () => {
        ...
        if (ref.current!.scrollLeft >= maxScrollWidth) {
            setState((value) => {
                return { ...value, isDisabledNext: true, isDisabledPrev: false }
            })
        } else if (ref.current!.scrollLeft < 1) {
            setState((value) => {
                return { ...value, isDisabledNext: false, isDisabledPrev: true }
            })
        } else {
            setState((value) => {
                return {
    ...
    return (
        <div className="relative">
            ...
            <button
                ...
                <InlineIcon icon={arrowLeft} width={20} height={20} />
            </button>
            <button
                ...
                <InlineIcon icon={arrowRight} width={20} height={20} />
            </button>
        </div>
    )
}
